...
function set_damage(msg, text, guild_room, player_stats, modules, commands, db) {
	...
	if (damage[0]) {
		...
		console.log('Guild:' + guild_room.settings.id + ' has set damage range to min:' + min + ', max:' + max);
		guild_room.settings.damage_range = {
			MIN: min,
			MAX: max
		};
		var list = JSON.stringify(guild_room.settings.damage_range);
		db.run('UPDATE Guilds SET DMG_RANGE =\'' + list + '\' WHERE ID ="' + guild_room.settings.id + '"');
...
function set_hp(msg, text, guild_room, player_stats, modules, commands, db) {
	...
	if (hp[0]) {
		...
		var min = parseFloat(num[0]);
		var max = parseFloat(num[1]);
		console.log('Guild:' + guild_room.settings.id + ' has set hp range to min:' + min + ', max:' + max);
		guild_room.settings.hp_range = {
			MIN: min,
			MAX: max
		};
		msg.channel.sendCode('cpp', 'HP range has been set to min: ' + min + ', max: ' + max);
		var list = JSON.stringify(guild_room.settings.hp_range);
		db.run('UPDATE Guilds SET HP_RANGE =\'' + list + '\' WHERE ID ="' + guild_room.settings.id + '"');
	}
}
...
function set_crit(msg, text, guild_room, player_stats, modules, commands, db) {
	...
	if (crit[0]) {
		...
		if (num > 1)
			...
			num = 0;
		console.log('Guild:' + guild_room.settings.id + ' has set crit rate to:' + num);
		guild_room.settings.crit_rate = num;
		msg.channel.sendCode('cpp', 'Critical rate has been set to: ' + num);
		db.run('UPDATE Guilds SET CRIT =' + num + ' WHERE ID ="' + guild_room.settings.id + '"');
	}
}
